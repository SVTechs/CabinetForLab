...
namespace Utilities.String
{
    ...
    {
        public static string EncodeUrlEx(string origin)
        {
            return HttpUtility.UrlEncode(origin.Replace("+", "%2b"));
        }

        public static string GetDownloadName(string fileName)
        {
            if (HttpContext.Current == null || HttpContext.Current.Request.UserAgent == null) return fileName;
        ...
        public static DateTime? GetBornDate(string identCode)
        {
            if (string.IsNullOrEmpty(identCode)) return null;
            if (identCode.Length == 18)
            {
                if (!DateTime.TryParse(identCode.Substring(6, 4) +
                                       "-" + identCode.Substring(10, 2) + "-" + identCode.Substring(12, 2),
                    out var time))
                {
                    return null;
                }
            ...
            if (identCode.Length == 15)
            {
                string timeBase = "19";
                if (!int.TryParse(identCode.Substring(6, 2), out var year))
                {
                    return null;
                }
                if (year < 30) timeBase = "20";
                if (!DateTime.TryParse(timeBase + identCode.Substring(6, 2) +
                                       "-" + identCode.Substring(8, 2) + "-" + identCode.Substring(10, 2),
                    out var time))
                {
                    return null;
                }
        ...
        {
            Match mInfo = Regex.Match(ch, @"[\u4e00-\u9fa5]");
            if (mInfo.Success)
            {
                return true;
            }

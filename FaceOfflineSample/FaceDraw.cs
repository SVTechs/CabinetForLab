...
namespace testface
{
    public class FaceDraw
    {
        ...
        public static int draw_rects(ref Mat img, int face_num, BDFaceBBox[] info)
        {
            ...
            {
                if (Single.IsNaN(info[i].center_x) || Single.IsNaN(info[i].center_y) ||
                    Single.IsNaN(info[i].width) || Single.IsNaN(info[i].height) ||
                    Single.IsInfinity(info[i].center_x) || Single.IsInfinity(info[i].center_y) ||
                    Single.IsInfinity(info[i].width) || Single.IsInfinity(info[i].height))
                {
                    return 0;
                }
                if (info[i].center_x > 1000000 || info[i].center_y > 1000000 || 
                    info[i].width > 1000000 || info[i].height > 1000000)
                {
                    return 0;
                }
